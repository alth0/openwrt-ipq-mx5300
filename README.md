...
| [24.10-nss-custom](https://github.com/alth0/openwrt-ipq-mx5300/tree/24.10-nss-custom)               |6.12.6|Current with upstream `openwrt-24.10` (next stable release) Also includes LuCI, nano, irqbalance, htop and an RGB LED controller utility (ledcontrol) for the onboard LED by default.|
...
2. Update feeds:
   ...
   ```
3. Generate the full config (Target Linksys MX5300 is already selected)
   ```bash
   make defconfig V=s
   ```
4. Now run full build
   ```bash
